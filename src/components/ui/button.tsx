...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-all duration-300 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-[var(--page-accent-color)] text-primary-foreground hover:brightness-110 transform hover:-translate-y-0.5 shadow-lg shadow-[var(--page-accent-color)]/30 hover:shadow-[var(--page-accent-color)]/50",
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90 transform hover:-translate-y-0.5",
        outline:
          "border border-input bg-background/50 hover:bg-[var(--button-hover-bg,hsl(var(--accent)))] hover:text-accent-foreground transform hover:-translate-y-0.5",
        secondary:
